...
// Copyright (c) 2013-2014 canadacoin Developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // canadacoin: starting difficulty is 1 / 2^12
...
{
    // canadacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // canadacoin: every 4 hours
static const int64 nTargetSpacing = 60; // canadacoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // canadacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // canadacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("canadacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("canadacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static canadacoinMiner(CWallet *pwallet)
{
    printf("canadacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("canadacoin-miner");
    ...
    try { loop {
        ...
        printf("Running canadacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("canadacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&canadacoinMiner, pwallet));
}
